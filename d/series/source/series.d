...
auto digits(string s) {
  import std.conv : parse, to;
  
  auto output = new int[s.length];
  int charIdx = 0;
  foreach(dchar c; s) {
    // Don't catch bad cast - pass to caller
    if (c < 48 || c > 57) {
...
auto slice(string s, int n) {
  if (n > s.length) {
    throw new Exception("window greater than string length");
  }
  
  auto result = new int[][](s.length-n+1, n);
  for (int i = 0; i < s.length - n + 1; i++) {
    result[i] = digits(s[i..(i+n)]);
  }
  return result;
}
